...
states <- read_csv("http://covidtracking.com/api/states/daily.csv") %>%
  ...
  bind_rows(mutate(read_csv("https://covidtracking.com/api/v1/states/current.csv"), date = lubridate::today("America/New_York"))) %>%
...
sc$new_deaths[sc$date > as.Date("2020-03-05") & sc$date < as.Date("2020-03-15")] <- 0
# sc$reporting_backlog <- FALSE
# sc$reporting_backlog[sc$date %in% c(as.Date("2020-04-30"), as.Date("2020-07-08"))] <- TRUE
...
sc %<>%
  ...
  mutate(
    ...
    pos_rate_roll = slide_dbl(pos_rate, ~mean(.x), .before = 6),
    ...
    hospitalizedCurrently = case_when(
      !is.na(in_hospital) ~ in_hospital,
      TRUE ~ hospitalizedCurrently
    ),
    hospitalizedCurrently = case_when(
      hospitalizedCurrently == 1593 ~ NA_real_,
      TRUE ~ hospitalizedCurrently
    ),
    hosp_roll = slide_dbl(hospitalizedCurrently, ~mean(.x), .before = 2),
    ever_hosp_roll = slide_dbl(hospitalizedCumulative, ~mean(.x), .before = 2),
...
ggplot(sc %>%
  ...
  geom_path(data = sc %>% pivot_longer(c(new_tests_roll, new_positives_roll), names_to = "test_type") %>%
         mutate(test_type = ifelse(test_type == "new_positives_roll",
                                   yes = "Positives", no = "Total"))) +
...
smooth1 <- paste0(
  ...
  "7-day average positives: ", round(10^gtests$x$data[[1]]$y)
)
smooth2 <- paste0(
  "Date: ", as.Date(round(gtests$x$data[[2]]$x), origin = "1970-01-01"), "\n",
  "7-day average tests: ", round(10^gtests$x$data[[2]]$y)
)
...
ggplot(sc, aes(x = date, y = new_deaths)) +
  ...
  # geom_smooth(se = F, color = "black", n = num_points) +
  geom_path(aes(y = new_deaths_roll)) +
  theme_nice(base_family = "IBM Plex Sans") +
  ylab("Deaths/day") +
  xlab("Date") +
  geom_vline(xintercept = as.numeric(as.Date("2020-05-04")),
...
ann <- list(
  x = c(as.numeric(as.Date("2020-04-16")), as.numeric(as.Date("2020-07-18"))),
  y = c(500, 750),
  xref = "x",
  yref = "y",
  text = c("Number of currently\nhospitalized not reported\nuntil May", 
           "Hospital reporting temporarily\nstopped in mid-July"),
  showarrow = FALSE,
  arrowhead = 6,
  arrowsize = 0.01,
  ax = -50,
...
ggplot(sc, aes(x = date, y = hospitalizedCumulative)) +
  ...
  # geom_smooth(se = F, color = "black", n = num_points) +
  geom_path(aes(y = ever_hosp_roll)) +
  theme_nice(base_family = "IBM Plex Sans") +
...
smooth1 <- paste0(
  ...
  "4-day average # total hospitalized = ", round(ghosp2$x$data[[2]]$y, 0)
)
...
inner_join(acs_dat, nytc, by = "fips") %>%
  ...
  scale_fill_gradient(low = "white", high = scales::muted("red"),
                      limits = c(10, 70), oob = scales::squish) -> scmap
