...
sc %<>%
  ...
  mutate(
    ...
    new_deaths_roll = slide_dbl(new_deaths, ~mean(.x), .before = 6),
    hospitalizedCurrently = case_when(
      !is.na(in_hospital) ~ in_hospital,
      TRUE ~ hospitalizedCurrently
    ),
    hosp_roll = slide_dbl(hospitalizedCurrently, ~mean(.x), .before = 6),
    ever_hosp_roll = slide_dbl(hospitalizedCumulative, ~mean(.x), .before = 6),
...
smooth1 <- paste0(
  ...
  "7-day average # in hospital = ", round(ghosp$x$data[[2]]$y, 0)
)
...
smooth1 <- paste0(
  ...
  "7-day average # total hospitalized = ", round(ghosp2$x$data[[2]]$y, 0)
)
